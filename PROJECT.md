...
To assess the performance success of the hardware acclerator implementation, a target execution time was calculated for each iteration. More details are visible in documentation for [Challenge 12](./challenge-09/README.md#iteration-two-estimations), but to summarize, the proof-of-concept v0.2.0 implementation (iteration two) did appear to meet the target execution time requirements. Importantly, this version also appeared to meet other desired performance characteristics such as relatively low power consumption and overall footprint (see full [metrics json file](./lstm_autoencoder-v0.2.0/verilog/openlane_results/metrics-16x16-40ns.json)). This conclusion comes with some caveats, however. The successful synthesis of the `matvec_mult` module had a reduced maximum vector input size of 16 rather than 64, as described above. This synthesis also did not include any SRAM macros, despite numerous attempts to determine the correct configuration for their inclusion (see the [OpenLane2 configuration with SRAM macros](./lstm_autoencoder-v0.2.0/verilog/config.json) for more detail). It should be assumed then, that the power requirements and footprint would be larger than the synthesis metrics described, once SRAM macros are successfully integrated. It is also assumed that usage of SRAM macros would allow for the increase in size back up to 64 as the orginal design intended, once the input vector could be buffered in SRAM. More details on interpretation of the synthesis results are found in the [lstm_autoencoder-v0.2.0 README](./lstm_autoencoder-v0.2.0/README.md#interpreting-synthesis-results).

The maximum number of clock cycles required for a pessimistic assumption of all matrix-vector multiplication operations using the maximum vector length of 64 was calculated to be 5885 cycles with a 40ns clock period, while simulation showed a 64x64 matrix-vector multiplication operation required 4223 cycles (seen in the [simulation-transcript](./lstm_autoencoder-v0.2.0/docs/simulation-transcript.txt)). Though this is not a large speedup, the target execution time estimate was intentionally pessimistic, since only a fraction of the matrix-vector multiplication operations performed in the actual algorithm would use the maximum vector size of 64. In addition, given that the stated goals for the project included reduced size and power consumption, the set of synthesis metrics taken in whole seem to point to a moderately successful result.
